...
def cal_weight_lineareg():
	...
	while(cal_poin < input_X_data_test.shape[0]):
		...
		# Error calculation
		error = (cal_Y_predction_values[cal_poin] - input_y_data_test[cal_poin]) ** 2

		cal_err.append(error)
	...
	plt.scatter(input_X_data_test, input_y_data_test, label='Test data 10 points')
	plt.scatter(input_X_data_train, input_y_data_train, label='Train data 20 points')
...
input_X_data_train = np.asarray([item[0] for item in training_data[:20]], dtype=float)
...
input_y_data_train= np.asarray([item[1] for item in training_data[:20]], dtype=float)
